describe Event::ValidateReservation do
  describe '.for' do
    ...
    it { is_expected.to eq(Reservation::Status::SUCCESS) }

    context 'when event is not free' do
      let(:status) { Event::Status::PENDING }

      it { is_expected.to eq(Reservation::Status::CANT_BOOK) }
    end
    ...
    context 'when user have reservation' do
      ...
      it { is_expected.to eq(Reservation::Status::RESERVED_OVERLAPS) }
    end
    ...
    context 'when user have booked his service at same time' do
      ...
      it { is_expected.to eq(Reservation::Status::OWNED_OVERLAPS) }
    end
  end
end
